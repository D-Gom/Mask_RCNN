...
class NucleusConfig(Config):
    ...
    STEPS_PER_EPOCH = (1000 - len(VAL_IMAGE_IDS)) // IMAGES_PER_GPU
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        for f in next(os.walk(mask_dir))[2]:
            if f.endswith(".png"):
                m = skimage.io.imread(os.path.join(mask_dir, f), as_gray=True).astype(bool)
                mask.append(m)

        if len(mask) == 0:
            img_dir = os.path.join(os.path.dirname(os.path.dirname(info['path'])), "images")
            for f in next(os.walk(img_dir))[2]:
              if f.endswith(".png"):
                  m = skimage.io.imread(os.path.join(img_dir, f), as_gray=True).astype(bool)
                  m[:,:] = False;
                  mask.append(m)
...
def detect(model, dataset_dir, subset):
    ...
    for image_id in dataset.image_ids:
        ...
        # muestra las imagenes con la mascaras y bbox
        visualize.display_instances(
            image, r['rois'], r['masks'], r['class_ids'],
            dataset.class_names, r['scores'],
